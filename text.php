...
function print_explanatory_text(InfinitiveVerb $infinitiveVerb) {
	global $verbes_en_ancien, $verbes_defectifs, $verbes_impersonnels, $verbes_transitifs, $verbes_intransitifs, $verbes_pronominaux, $unregelmaessige_verben, $category;
	...
	if ((substr ( $infinitiveVerb, - 2, 2 ) == 'er') && (! in_array ( $infinitiveVerb, [ 
			'aller',
			'raller',
			're-aller',
			'saisir-arrêter',
	...
	} elseif ((substr ( $infinitiveVerb, - 2, 2 ) == 'ir') or (in_array ( mb_substr ( $infinitiveVerb, - 2, 2, "utf-8" ), [ 
			"ïr" 
	] )) && ! in_array ( $infinitiveVerb, $unregel_ir )) {
	...
	echo '<p class="well">Das Verb <b>' . $_GET ['verb'] . '</b> wird <b>' . $regelmaessig . '</b> konjugiert und wird in den zusammengesetzten Zeiten mit den Hilfsverb <b>' . finding_auxiliaire ( new InfinitiveVerb ( $_GET ['verb'] ) )->getValue () . '</b> gebildet.</p>
<p class="well">' . $_GET ['verb'] . ' ist ein <b>' . $reflexiv . '</b> und <b>' . $transitiv . '</b> Verb und ist in <b>' . $group . '</b>.</p>';
